########################################
# Coded by Ubaldo Gonzalez			       #
# RedID: 824311417				             #
# Dates: 06/01/2020				             #
# Game title: Snake Xtreme             #
########################################

.data
##########################################################################
# Important data stored in this section such as display settings and other items in the game #
##########################################################################

# This helps with storing the position of the snake moving horizontally and vertically
	head_horizontal:	.half 5			# This stores the position of the player in the memory
	head_vertical:		.half 5	
	tailX:			.half 4
	tailY:			.half 5
	...
	food01X:		.half 30		# Store initial foods' positions in memory
	food01Y:		.half 30
	food02X:		.half 60
	food02Y:		.half 60
...
# Screen size display settings
 	heightofScreen:		.half 32		# Usual settings $gp, 32x32, 16xScaling, 512x512
	widthofScreen:		.half 32		# Store stage size
...
Main_init:
			# Load player info
			lh $a0, head_horizontal		# Get player's head possition
			lh $a1, head_vertical			
			...
			or $a1, $zero, $s1		# Load position
			jal PaintMemory			# Call the paint function
			nop
 
			# Draw initial food
			jal GetFoodPos			# Get a random position for the new food
			nop
			ori $a0, $zero, 0x32FFFFFF	# Load white as the colour and attach the food bytes
			or $a1, $zero, $v0		# Load position
			jal PaintMemory			# Call the paint function
			nop
...
Main_gameLoop:
			...
			jal Sleep			# Sleep for 60 milliseconds so that the game is playable
			nop
...
Main_tailBegin:
			...
			or $a0, $zero, $s7		# Remove tailpiece
			or $a1, $zero, $s1		# Load address
			jal PaintMemory			# Call the paint function
			nop
...
Main_tailNone:
			...
			syscall
Main_tailDone:
			or $s1, $zero, $v0		# Store player's new tail position
...
CoordsToAddress:
		...
		lh $a0, widthofScreen		# Load the screen width into a0
...
# Returns none
FillMemory:
		lh $a1, widthofScreen		# Calculate ending possition
		lh $a2, heightofScreen
...
AddBoundaries_t:	
		...
		lh $a1, widthofScreen		# Calculate next ending condition
		lh $a2, heightofScreen
		multu $a1, $a2			# Multiply screen width by screen height
		nop
		mflo $a2			# Retreive total tiles
...
AddBoundaries_s:	
		...
		lh $a1, widthofScreen		# Calculate final ending possition
		lh $a2, heightofScreen
		multu $a1, $a2			# Multiply screen width by screen height
		nop
		mflo $a2			# Retreive total tiles
...
GetFoodPos:
		lh $a1, widthofScreen		# Calculate the max
		lh $a2, heightofScreen
		multu $a1, $a2			# Multiply screen width by screen height
		nop
